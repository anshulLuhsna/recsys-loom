...
| LightGBM truncation | aligning the objective with Top-12 helps | lambdarank_truncation_level 12 / 20 | 0.02313 / 0.02538 vs baseline 0.02707 | not inspected | reject so far | worse than default truncation | pending |
| hybrid search baseline | metadata + BM25 can answer structured catalog queries | intent + BM25 + structured (+ optional MiniLM) | synthetic structured P@10=1.0 (tautological); style spotchecks retrieve on-type items | n/a | keep as search baseline | no query logs; label as synthetic | pending |
